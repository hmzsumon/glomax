...
const ReferRecords = () => {
	...
	// filter records with purpose bonus and trade_commission
	const filteredRecords = records?.filter((record: any) => {
		return record.purpose === 'bonus' || record.purpose === 'trade_commission';
	});
			...
			<div className='mx-auto'>
				<div className='w-full h-full bg-black_2'>
					...
					<CardFooter className='flex items-center justify-between w-full p-4 border-t border-blue-gray-50'>
						...
						>
							Page {currentPage} of {Math.ceil(filteredRecords?.length / 5)}
						</Typography>
						<div className='flex gap-2'>
							...
							<Button
								...
								size='sm'
								disabled={
									currentPage === Math.ceil(filteredRecords?.length / 5) ||
